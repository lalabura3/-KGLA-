"""Note & NoteSection ORM models — AI-generated structured notes.

One Note per processed video, with time-anchored sections linked to
ASR segments.  Includes hallucination scoring for quality tracking.
"""
...
class Note(Base, UUIDPKMixin, TimestampMixin):
    """Top-level AI-generated note — one per processed video."""
...
class NoteSection(Base, UUIDPKMixin, TimestampMixin):
    """Individual section of a note, anchored to a video time range."""
    ...
    segment_ids: Mapped[list[str] | None] = mapped_column(
        JSONB, nullable=True,
    )
    ...
    hallucination_flags: Mapped[list[str] | None] = mapped_column(
        JSONB, nullable=True,
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<NoteSection #{self.section_index}"
            f" [{self.start_time:.1f}s] {self.heading}>"
        )
