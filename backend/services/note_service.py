...
class NoteService:
    ...
    @property
    def provider(self):
        """Lazy-load the LLM provider from factory on first use."""
        if self._provider is None:
            from backend.providers import _lazy_init
            self._provider = _lazy_init()
    ...
    async def _call_llm(
        ...
        """Call the LLM provider (retry is handled by the provider)."""
